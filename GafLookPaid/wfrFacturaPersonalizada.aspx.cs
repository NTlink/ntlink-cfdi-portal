...
{
    public partial class wfrFacturaPersonalizada : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                try
                {
                    ...
                    var listaDetalles = new List<facturasdetalle>();
                                       
                    ViewState["detalles"] = listaDetalles;//new List<facturasdetalle>();
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            {
                Redondear = true, //Solo en esta aplica
            ...
            //if (cbIva.Checked) //no lleva la opcion de selecion de iva
            {
                detalle.PorcentajeIva = int.Parse("16");
        ...
        private void GuardarFactura()
        {
            ...
            if (ValidarFactura())
            {
                ...
                try
                {
                    foreach (var facturasdetalle in detalles)
                    {
                        facturasdetalle.Redondear = true;
                    }
        ...
        private void Clear()
        {
            this.txtCodigo.Text = this.txtDescripcion.Text = this.txtDetalles.Text = this.txtPrecio.Text = this.txtOrdenCompra.Text =
        ...
        private void UpdateTotales()
        {
            ...
            var total = 0M;
            var subtotal = 0M;
            const double multIva = 0.16;

            foreach (facturasdetalle detalle in detalles)
            {
                var partida = decimal.Round(detalle.TotalPartida, 2, MidpointRounding.AwayFromZero);
                subtotal += partida;
            }
            iva = subtotal*0.16M;
            total = iva + subtotal;
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (edicion.TotalPartida != null)
            edicion.ImporteIva = ((decimal)16 / 100) * decimal.Round(edicion.TotalPartida, 2, MidpointRounding.AwayFromZero);
            ViewState["detalles"] = detalles;
            this.BindDetallesToGridView();
            this.UpdateTotales();
        }
