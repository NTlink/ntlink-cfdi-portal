...
{
    public partial class wfrFacturaAddendaPlaneta : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                try
                {
                    ...
                    ViewState["DetalleADDOC"] =new List<DetalleADDOC>();

                    BindAddendaPlanetaGridView();
                    ViewState["iva"] = 0M;
                    ViewState["total"] = 0M;
        ...
        {
            //this.ClearAll();
            Response.Redirect("wfrFacturaPilgrims.aspx");
        }
        ...
        {
            if ((ViewState["DetalleADDOC"] as List<DetalleADDOC>).Count == 0)
            {
                this.lblError.Text = "La addenda no puede estar vacía";
                return false;
            }
        ...
        private facturas GetFactura(string iniciales, List<facturasdetalle> detalles)
        {
            ...
            {
                TipoDocumentoSAT = TipoDocumento.AddendaPlaneta,
                TipoDocumento = TipoDocumento.AddendaPlaneta,
             ...
             */
            fact.addendaPlaneta = GetDatosAddendaPlaneta();
        ...
        private AddendaPlanetaV1 GetDatosAddendaPlaneta()
        {

            var ap = new AddendaPlanetaV1();
            ap.Empresa = ddlEmpresaAP.SelectedValue;
            ap.FechaADDOC = txtFechaADDOC.Text;
            ap.Folio = txtFolioAP.Text;
            ap.Requisicion = txtRequisicion.Text;
            ap.detalleADDOC = ViewState["DetalleADDOC"] as List<DetalleADDOC>;
        ...
        private void BindAddendaPlanetaGridView()
        {
            var conceptos = ViewState["DetalleADDOC"] as List<DetalleADDOC>;
             
             if (conceptos != null && conceptos.Count > 0)
            {
                int noColumns = this.gvAddendaPlaneta.Columns.Count;
                this.gvAddendaPlaneta.Columns[noColumns - 1].Visible = this.gvAddendaPlaneta.Columns[noColumns - 2].Visible = true;
            }
            else
            {
                int noColumns = this.gvAddendaPlaneta.Columns.Count;
                this.gvAddendaPlaneta.Columns[noColumns - 1].Visible = this.gvAddendaPlaneta.Columns[noColumns - 2].Visible = false;
            }
             this.gvAddendaPlaneta.DataSource = conceptos;
             this.gvAddendaPlaneta.DataBind();
        }
        ...
        */
        ...
        {
            Response.Redirect("wfrFacturaAddendaPlaneta.aspx");
        }




        protected void btnAgregarAddendaPlaneta_Click(object sender, EventArgs e)
        {
            var DP = ViewState["DetalleADDOC"] as List<DetalleADDOC>;
            if (DP == null)
            {
                DP = new List<DetalleADDOC>();
                ViewState["DetalleADDOC"] = DP;
            }
            var p = new DetalleADDOC
            {
                Linea =Convert.ToInt32(txtLinea.Text),
               Cantidad = txtCantidadAP.Text,
               Codigo = txtCodigoAP.Text,
               Descripcion=txtDescripcionAP.Text,
               HojaTrabajo=txtHojaTrabajo.Text,
               ImporteTotal=txtImporteTotal.Text,
               PrecioUnitario=txtPrecioUnitarioAP.Text,
               ProcesoProd=ddlProcesoProd.SelectedValue,
            ...
            ViewState["DetalleADDOC"] = DP;
             txtCantidadAP.Text="";
             txtCodigoAP.Text="";
             txtDescripcionAP.Text="";
             txtHojaTrabajo.Text="";
             txtImporteTotal.Text="";
             txtPrecioUnitarioAP.Text="";
             txtTipoTrabajo.Text = "";
             txtLinea.Text = "";
        ...
        protected void gvAddendaPlaneta_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "EliminarPilgrims")
            {
                var DP = ViewState["DetalleADDOC"] as List<DetalleADDOC>;
                DP.RemoveAt(Convert.ToInt32(e.CommandArgument));
                this.gvAddendaPlaneta.DataSource = DP;
                this.gvAddendaPlaneta.DataBind();
        ...
        protected void txtPedimento_TextChanged(object sender, EventArgs e)
        {
                    }
