...
{
    public partial class wfrFacturaAddenda : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                try
                {
                    ...
                    ViewState["ConceptosLargos"] = new List<string>();
        ...
        {
            //this.ClearAll();
            Response.Redirect("wfrFacturaAdo.aspx");
        }
        ...
        private facturas GetFactura(string iniciales, List<facturasdetalle> detalles)
        {
            ...
            {
                TipoDocumentoSAT = TipoDocumento.AddendaConcep,
                TipoDocumento = TipoDocumento.AddendaConcep,
             ...
             */
            fact.addendaConcepto = GetDatosFacturaConcepto();
            if (ddlMoneda.SelectedValue == "MXN")
        ...
        private AddendaConcepto GetDatosFacturaConcepto()
        {

            AddendaConcepto dc = new AddendaConcepto();
            var con=ViewState["ConceptosLargos"] as List<string>;
            dc.Concepto = con;
            return dc;
        }
        ...
        private void BindConceptosLargosToGridView()
        {
            List<string> detalles = ViewState["ConceptosLargos"] as List<string>;
            if (detalles != null && detalles.Count > 0)
            {
                int noColumns = this.gvConceptoLargo.Columns.Count;
                this.gvConceptoLargo.Columns[noColumns - 1].Visible = this.gvConceptoLargo.Columns[noColumns - 2].Visible = true;
            }
            else
            {
                int noColumns = this.gvConceptoLargo.Columns.Count;
                this.gvConceptoLargo.Columns[noColumns - 1].Visible = this.gvConceptoLargo.Columns[noColumns - 2].Visible = false;
            }
            DataTable table = new DataTable();
            table.Columns.Add("ID");
            table.Columns.Add("Concepto");
            int t = 0;
            foreach (var array in detalles)
            {
            ...
            this.gvConceptoLargo.DataSource = table;
            this.gvConceptoLargo.DataBind();
        }
        ...
        */
        ...
        {
            Response.Redirect("wfrFacturaAddenda.aspx");
        }
        ...
        protected void btnAgregarConceptoLargo_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtConceptoLargo.Text))
            {
                List<string> CFDIConceptos = ViewState["ConceptosLargos"] as List<string>;
                CFDIConceptos.Add(txtConceptoLargo.Text);

                ViewState["ConceptosLargos"] = CFDIConceptos;
                BindConceptosLargosToGridView();
                txtConceptoLargo.Text = "";
            }
        }

        protected void gvConceptoLargo_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("EliminarConceptosLargos"))
            {
                var CFDIConceptos = ViewState["ConceptosLargos"] as List<string>;
                CFDIConceptos.RemoveAt(Convert.ToInt32(e.CommandArgument));
                ViewState["ConceptosLargos"] = CFDIConceptos;
                this.BindConceptosLargosToGridView();
