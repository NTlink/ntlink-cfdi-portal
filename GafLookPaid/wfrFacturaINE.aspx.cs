...
{
    public partial class wfrFacturaINE : Page
    {
            ...
            if (!this.IsPostBack)
            {
                try
                {
                    ...
                    using (cliente as IDisposable)
                    {
                        ...
                        ddlClaveUnidad.DataSource = cliente.ConsultarClaveUnidadAll();
                        ...
                        //--------------------------------------------
                        ddlMoneda.DataSource = cliente.Consultar_MonedaAll();
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            facturasdetalle detalle = new facturasdetalle()
            {
                ...
                Precio = decimal.Parse(this.txtPrecio.Text), /*Decimal.Round(decimal.Parse(this.txtPrecio.Text), mon),*/
        ...
        private void ClearAll()
        {
            ...
            this.txtProyecto.Text = this.txtFolioOriginal.Text = this.txtFechaOriginal.Text = this.txtMontoOriginal.Text =
                //this.txtMetodoPago.Text = this.txtDiasRevision.Text =  this.txtCuenta.Text = 
        ...
        private void BindContabilidadToGridView()
        {
            List<INEContabilidad> INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;
            if (INEContabilidad != null && INEContabilidad.Count > 0)
            {
                int noColumns = this.gvContabilidad.Columns.Count;
                this.gvContabilidad.Columns[noColumns - 1].Visible = this.gvContabilidad.Columns[noColumns - 2].Visible = true;
            }
            else
            {
                int noColumns = this.gvContabilidad.Columns.Count;
                this.gvContabilidad.Columns[noColumns - 1].Visible = this.gvContabilidad.Columns[noColumns - 2].Visible = false;
            }


            this.gvContabilidad.DataSource = INEContabilidad;
            this.gvContabilidad.DataBind();
        }
        ...
        */
        ...
        protected void btnAgregarContabilidad_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(ddlID.SelectedValue) && !string.IsNullOrEmpty(txtIdContabilidadC.Text))
            {
                List<INEContabilidad> INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;

                INEContabilidad Contabilidad = new INEContabilidad();
                Contabilidad.IdContabilidad = txtIdContabilidadC.Text;
                Contabilidad.ID = ddlID.SelectedValue;
                INEContabilidad.Add(Contabilidad);
                ViewState["INEContabilidad"] = INEContabilidad;
                this.BindContabilidadToGridView();
                txtIdContabilidadC.Text = "";
            }
        }

        protected void gvContabilidad_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("EliminarContabilidad"))
            {
                var INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;
