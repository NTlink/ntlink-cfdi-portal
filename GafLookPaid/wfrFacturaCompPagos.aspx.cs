...
namespace GafLookPaid
{
    public partial class wfrFacturaCompPagos : Page
    {
        ...
        {
            //this.ClearAll();
            Response.Redirect("wfrFacturaCompPagos.aspx");
        }
            ...
            if (ValidarFactura())
            {
                ...
                try
                {
                    ...
                    using (clienteServicio as IDisposable)
                    {
                        ...
                        comple.pagos = new List<Pagos>();
                        List<Pagos> pagos = new List<Pagos>();
                         pagos = ViewState["Pagos"] as List<Pagos>;
                        List<PagoDoctoRelacionado> documentos = new List<PagoDoctoRelacionado>();
                        documentos = ViewState["PagoDoctoRelacionado"] as List<PagoDoctoRelacionado>;
        ...
        */
        ...
        protected void Fecha_Sello(int idEmp) {
            try
            {
                ...
                if (!string.IsNullOrEmpty(idEmpresaString) && int.TryParse(idEmpresaString, out idEmpresa))
                {
                    ...
                    //TimeSpan c = FechaVence - DateTime.Parse("23/11/2020 01:24:53 p. m.");
        ...
        {
            int tamaño = txtDescripcion.Text.Length;
            int max = txtDescripcion.MaxLength;
            rest.Text = max - tamaño + " / " + max;
        }
