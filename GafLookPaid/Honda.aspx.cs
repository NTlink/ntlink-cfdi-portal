...
namespace GafLookPaid
{
    public partial class Honda : System.Web.UI.Page
    {
        ...
        private bool ValidarFactura()
        {
            ...
            //----------------grv
            if (string.IsNullOrEmpty(txtNumber.Text) && string.IsNullOrEmpty(txtReferenceNumber.Text))
            {
                this.lblError.Text = "Escribe el ASNNumber ó ReferenceNumber";
                    return false;
            }
            if (!string.IsNullOrEmpty(txtNumber.Text) && !string.IsNullOrEmpty(txtReferenceNumber.Text))
            {
                this.lblError.Text = "Escribe solo un ASNNumber ó ReferenceNumber";
                return false;
            }
            if (string.IsNullOrEmpty(txtnoproveedor.Text))
            {
                this.lblError.Text = "Escribe el numero de proveedor";
                return false;
            }
            
            if (string.IsNullOrEmpty(txtShioTo.Text))
            {
                this.lblError.Text = "Escribe el ShipTo";
                return false;
            }
        ...
        private facturas CrearFactura(List<facturasdetalle> detalles)
        {
            ...
            {
                TipoDocumento = TipoDocumento.Honda,
            ...
            //datos AddendaHonda---------------rgv
             fact.FacturasAddendaHonda = GetDatosFacturaAddenda();
             string fecmodi = fact.Fecha.Year.ToString() + fact.Fecha.Month.ToString() + fact.Fecha.Day.ToString();
             fact.FacturasAddendaHonda.fecha = fecmodi;
        ...
        private DatosAddendaHonda GetDatosFacturaAddenda()
        {
            //campos txt factura addenda honda
            DatosAddendaHonda dfa = new DatosAddendaHonda
            {
                fecha="",
                folio =txtFolio.Text,
                ASNNumber=txtNumber.Text,
                moneda = ddlMoneda.SelectedItem.Text, 
                tipoComprobante="",
                tipoDocumento = ddlTipoDocumento.SelectedValue, 
                rfc="",
                ShipTo=txtShioTo.Text, 
                numeroProveedor=txtnoproveedor.Text,     
                ReferenceNumber=txtReferenceNumber.Text,
                PlantCode= ddlPlantCode.SelectedValue
